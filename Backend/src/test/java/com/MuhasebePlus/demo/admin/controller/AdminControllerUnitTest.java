...
class AdminControllerUnitTest {
    ...
    void userEndpoints_delegateAndReturnExpectedStatuses() {
        ...
        UpdateUserRoleRequestDto roleRequest = new UpdateUserRoleRequestDto(
            "USER"
        );
        AdminResetPasswordRequestDto passwordRequest =
            new AdminResetPasswordRequestDto("newSecret1");
        when(
            adminUserService.getUsers("adm", "ADMIN", "ACTIVE", pageable)
        ).thenReturn(page);
        ...
        assertThat(
            adminUserController
                .getUsers("adm", "ADMIN", "ACTIVE", pageable)
                .getBody()
        ).isSameAs(page);
        assertThat(adminUserController.getUser(10L).getBody()).isEqualTo(user);
        assertThat(
            adminUserController.updateRole(10L, roleRequest).getBody()
        ).isEqualTo(user);
        assertThat(adminUserController.lockUser(10L).getStatusCode()).isEqualTo(
            HttpStatus.NO_CONTENT
        );
        assertThat(
            adminUserController.unlockUser(10L).getStatusCode()
        ).isEqualTo(HttpStatus.NO_CONTENT);
        assertThat(
            adminUserController.activateUser(10L).getStatusCode()
        ).isEqualTo(HttpStatus.NO_CONTENT);
        assertThat(
            adminUserController.deactivateUser(10L).getStatusCode()
        ).isEqualTo(HttpStatus.NO_CONTENT);
        assertThat(
            adminUserController
                .resetPassword(10L, passwordRequest)
                .getStatusCode()
        ).isEqualTo(HttpStatus.NO_CONTENT);
        assertThat(
            adminUserController.deleteUser(10L).getStatusCode()
        ).isEqualTo(HttpStatus.NO_CONTENT);
    ...
    void companyEndpoints_delegateAndReturnExpectedStatuses() {
        ...
        AdminCompanyDetailDto detail = new AdminCompanyDetailDto(
            company,
            List.of(userDto(10L, "u@test.com", "USER")),
            5,
            7,
            new AdminCompanyDetailDto.AiQuotaDto(
                1000,
                250,
                LocalDateTime.of(2026, 7, 1, 0, 0)
            )
        );
        Page<AdminCompanyDto> page = new PageImpl<>(
            List.of(company),
            pageable,
            1
        );
        when(
            adminCompanyService.getCompanies("test", true, pageable)
        ).thenReturn(page);
        when(adminCompanyService.getCompanyDetail(1L)).thenReturn(detail);

        assertThat(
            adminCompanyController
                .getCompanies("test", true, pageable)
                .getBody()
        ).isSameAs(page);
        assertThat(
            adminCompanyController.getCompanyDetail(1L).getBody()
        ).isEqualTo(detail);
        assertThat(
            adminCompanyController.activateCompany(1L).getStatusCode()
        ).isEqualTo(HttpStatus.NO_CONTENT);
        assertThat(
            adminCompanyController.deactivateCompany(1L).getStatusCode()
        ).isEqualTo(HttpStatus.NO_CONTENT);
    ...
    void statsEndpoint_returnsServiceStats() {
        AdminStatsDto stats = new AdminStatsDto(
            ...
            3,
            true,
            List.of(new AdminStatsDto.AiConsumerDto(1L, "Test", 200, 1000)),
            List.of()
        );
    ...
    void logEndpoints_delegateAndExportCsvWithHeaders() {
        ...
        when(
            adminLogService.exportLogsAsCsv(
                1L,
                LogLevel.INFO,
                10L,
                true,
        ...
        assertThat(
            adminLogController
                .getLogs(1L, LogLevel.INFO, 10L, true, start, end, pageable)
                .getBody()
        ).isSameAs(page);

        ResponseEntity<byte[]> response = adminLogController.exportLogs(
            1L,
            LogLevel.INFO,
            10L,
            true,
        ...
        ).isEqualTo("admin_logs.csv");
        assertThat(response.getHeaders().getContentLength()).isEqualTo(
    ...
    private AdminCompanyDto companyDto(
        Long id,
        boolean active,
        long userCount
    ) {
