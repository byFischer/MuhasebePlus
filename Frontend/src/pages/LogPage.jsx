...
export default function LogPage() {
  ...
  const filtered = list.filter(x => (x.message + (x.actorName || '')).toLowerCase().includes(q.toLowerCase()));
      ...
      <div className="card">
        ...
        <div>
          <div className="log-row" style={{ background: 'var(--bg-elev)', fontWeight: 500, color: 'var(--ink-3)', textTransform: 'uppercase', fontSize: 10, letterSpacing: '0.05em' }}>
            <div>Tarih/Saat</div><div>Seviye</div><div>Modül</div><div>Mesaj</div>
          </div>
          {filtered.map((x, i) => (
            <div key={i} className="log-row">
              <div className="ts">{x.timestamp || x.createdAt}</div>
              <div className={`lvl ${x.level || x.logLevel}`}>{x.level || x.logLevel}</div>
              <div><span className="pill">{x.module || x.entityType || '—'}</span></div>
              <div><span>{x.message}</span>{x.actorName && <span className="actor"> · {x.actorName}</span>}</div>
            </div>
          ))}
